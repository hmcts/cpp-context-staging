...
import static uk.gov.moj.cpp.staging.integrationTest.utils.WiremockTestHelper.getHitsCountOnMockedEndpoint;
...
public class CreateUserIdamIT extends BaseIT {

    private static final Logger LOGGER = LoggerFactory.getLogger(CreateUserIdamIT.class);
    private static final String ACCOUNT_ENROLMENT_CHANGED_CONTENT_TYPE = "application/vnd.identity.events.account-enrolment-changed+json";
    private String WIREMOCK_VERIFY_URL = "/usersgroups-service/command/api/rest/usersgroups/users/";
    private static final String COMMAND = "json/identity/identity.events.account-enrolment-changed.json";
    ...
    public void sendRequestAndWaitForEvent() throws Exception {
        int wiremockCallCountBeforeTest = getHitsCountOnMockedEndpoint(WIREMOCK_VERIFY_URL);

        LOGGER.info("CALL COUNT BEFORE TEST: {}", wiremockCallCountBeforeTest);

        JSONObject payload = new JSONObject(getPayload(COMMAND));

        Response response = restClient.postCommand(BASE_URI + EVENT_BASE_URL + RESOURCE, ACCOUNT_ENROLMENT_CHANGED_CONTENT_TYPE, payload.toString(), idamHelper.getHeaders());
        ...
        IdamHelper.verifyPostSetUserDetails("1fc69990-bf59-4c4a-9489-d766b9abde9a","User");
