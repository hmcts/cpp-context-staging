...
TRANSFORMATION_JAR=$(echo target/staging-domain-transformations-anonymise*.jar)
...
[[ ! -f ${EVENT_TOOL_JAR} ]] && \
    curl -k ${mavenArtifactBaseUrl}/artifactory/repocentral/uk/gov/justice/event-tool/${EVENT_TOOL_VERSION}/event-tool-${EVENT_TOOL_VERSION}-swarm.jar  > ${EVENT_TOOL_JAR}
